...
- **OOV fallback:** the property model has a character-level branch so any *word* the model has not seen still produces plausible properties. Never refuse to commit because **the model** hasn't seen a word — the branch scores ~67,000 words beyond the 10,749-word vocabulary.
  - **Changed in Phase 3: strings that are not words are refused**, against a shipped lexicon of ~78,000 (`src/ml/lexicon.v1.txt`, built by `model/build_lexicon.py`). The character branch was doing two jobs and failing one of them: measured, `asdf` scored warmth +0.66, which is the model inventing a feeling for a string that has none. The lexicon answers "is this a word?"; the branch answers "what does it feel like?".
  - The lexicon is a real dictionary **intersected** with a frequency list, and both halves are load-bearing. Frequency alone ranks `asdf`, `wasd` and `qwerty` above `gloaming` — mashing is common on the web — and growing it admits *more* mashing, not less. A lexicon alone is 370k entries and 867 KB. The intersection is 78k words and 157 KB, admits zero of fourteen mashing probes, and still accepts `gloaming`, `tarn` and `scree`.
  - Input is letters-only, refused at the keystroke. Numbers, punctuation and anything over 24 characters never enter the buffer.
